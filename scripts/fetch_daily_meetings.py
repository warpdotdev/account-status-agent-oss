...
def main():
    ...
    # Grain API date params are unreliable, so we fetch recent recordings
    # and filter client-side by date. Results are newest-first, so pagination
    # stops once we've walked past the target date.
