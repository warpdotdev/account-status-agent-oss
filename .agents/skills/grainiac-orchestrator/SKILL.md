...
- **Timezone handling:** Grain timestamps are UTC. When the script resolves `today`, it uses the `GRAINIAC_TIMEZONE` env var (default: `America/Los_Angeles`) to determine the correct calendar date. This matters for evening Pacific runs where UTC has already rolled to the next day. Any IANA timezone name works, and DST is applied correctly for the current date. When a specific `YYYY-MM-DD` date is provided, the script matches recordings whose UTC `start_datetime` falls on that calendar date.
- The Grain API's `title_search` parameter does not filter server-side. Filtering happens client-side in `grain_client.py`.
- Company names are inferred from external participant email domains. If the inferred name is wrong (e.g., "Gmail" for a personal email), the child agent should correct it during analysis. Pass the corrected name in the child's prompt when the meeting title makes the real company obvious, and tell the child to confirm it against the transcript.
- **Rate limits:** The Grain API allows roughly 30 requests per window and paginates at 20 recordings per page. `grain_client.py` paces requests and retries on HTTP 429 with backoff, and the fetch script stops paginating once it passes the target date. Child agents also call the API concurrently, so if you add your own Grain calls, reuse `grain_client.py` rather than issuing raw requests.
- **Notion configuration:** If a Notion call fails with a 404, the database ID is either wrong or the database has not been shared with the integration. Verify what the token can reach by POSTing to `/search` with `{"filter": {"value": "database", "property": "object"}}` before assuming the target is missing, and confirm a candidate database by checking that its existing pages' bodies follow the template in `references/notion-template.md` — the database's own columns are unrelated to the template. Do not write meeting data into a database you have not confirmed this way.
